...
/**
 * UserManager tests for PostgreSQL.
 *
 * These tests require CREATE USER and DROP USER privileges.
 * Tests will be skipped if permissions are insufficient.
 */
final class UserManagerTests extends BasePostgreSQLTestCase
{
    public function setUp(): void
    {
        parent::setUp();

        // Clean up test user if exists (dropUser now uses IF EXISTS)
        try {
    ...
    {
        // Clean up test user (dropUser now uses IF EXISTS)
        try {
            UserManager::drop('test_user_manager', null, static::$db);
        } catch (\Throwable $e) {
