...
/**
 * JsonTests tests for mariadb.
 */
final class JsonTests extends BaseMariaDBTestCase
{
    ...
    public function testJsonMethodsEdgeCases(): void
    {
        ...
        ->where('id', $ids[1])
        ->getOne();
        // MariaDB uses JSON_REMOVE which actually removes the element (doesn't preserve array length)
        // After removal, index 0 now points to the next element (which was at index 1)
        // So we check that the array still exists and has elements
        $extraArray = $db->find()
        ->table('t_json_edge')
        ->selectJson('meta', ['extra'], 'extra')
        ->where('id', $ids[1])
        ->getOne();
        // The element was removed, so index 0 now points to what was index 1
        // We verify the array still exists (not null or empty)
        $this->assertNotNull($extraArray['extra'] ?? null);
        $this->assertNotEmpty($extraArray['extra'] ?? null);
