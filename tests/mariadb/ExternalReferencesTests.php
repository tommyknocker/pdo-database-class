...
/**
 * ExternalReferencesTests tests for mariadb.
 */
final class ExternalReferencesTests extends BaseMariaDBTestCase
{
    ...
    public function testRawValueStillWorks(): void
    {
        ...
        ->whereExists(function ($query) {
            $query->from('orders')
            ->where('user_id', new RawValue('users.id'))  // Explicit RawValue
            ->where('amount', 50, '>');
        })
        ->get();
